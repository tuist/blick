...
pub(super) fn update_latest_pointer(runs_root: &Path, run_id: &str) {
...
mod tests {
    ...
    fn pointer_targets_the_named_run() {
        ...
        update_latest_pointer(tmp.path(), "20990101T000000Z");
    ...
    fn pointer_replaces_an_existing_pointer() {
        let tmp = tempfile::tempdir().unwrap();
        update_latest_pointer(tmp.path(), "first");
        update_latest_pointer(tmp.path(), "second");
        let latest = tmp.path().join("latest");
        #[cfg(unix)]
        {
            let target = std::fs::read_link(&latest).unwrap();
