...
Findings whose `(file, line)` is **outside** the pull request's diff hunks (the agent commented on context it read) are surfaced in the per-`(scope, review)` check-run summary instead of as inline comments. GitHub rejects out-of-diff comments; this filter keeps the request safe.

`--run` selects which run to render (defaults to `latest`). `--head-sha` is required for `github-review` and `check-run` (in GitHub Actions, pass the pull request head commit).

## 🐙 GitHub Actions integration

`blick publish` reads the latest run from `.blick/runs/latest`, auto-detects the pull request context from the GitHub Actions environment, and posts a Check Run per `(scope, review)` plus one resolvable pull request review. See [`.github/workflows/blick.yml`](.github/workflows/blick.yml) for the canonical example. In this repository, the review job depends on the format, build, and test jobs, so the agent is invoked only after those checks pass. The review and publish portion is two steps:
...
- name: Publish to the pull request
  if: always()
  env:
...
The pull request author can mark each individual line comment as resolved as they fix it. That's standard GitHub pull request review behavior, and works because we post through the review endpoint rather than as plain issue comments or check annotations. The review carries no top-level summary body, since severity already shows on each line comment and on the check-run conclusion, so a duplicate "### Blick review" block on the conversation tab would just stack up one entry per push. Findings whose lines fall outside the pull request's diff are surfaced in the per-(scope, review) check-run summary instead.

It's also worth scoping the workflow to pull requests from branches inside the repository (`if: github.event.pull_request.head.repo.full_name == github.repository`). Forked pull requests don't have access to repo secrets and shouldn't be reviewed by a language model agent that can read the codebase before the change is trusted.
